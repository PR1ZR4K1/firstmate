...
For Lavish that verdict covers every completed feedback poll, an ended session, and a missing session; ordinary open-session feedback is explicitly re-armed with `--agent-reply` only after handler revision and durable reconciliation, so a plain poll cannot race ahead of the response that re-enables browser feedback.
