...
`bin/fm-procevent.sh` owns the generic contract; `bin/fm-procevent-lavish.sh` is the first adapter and wraps only the currently published `lavish-axi poll` interface through `bin/fm-lavish-review.sh`'s private runtime envelope.
Lavish runtime state lives at owner-only `state/lavish-axi/`, while sequence-keyed reply receipts live at owner-only `state/lavish-continuations/`; neither directory exists until a review command needs it.
The runtime envelope replaces ambient Lavish state, bind, link, allowed-host, port, telemetry, and publication-token values with one physical-home identity, and the continuation adapter never stores a reply in process registration argv.
...
Whether a captured result makes its current registration terminal is adapter knowledge, never the runner's.
After attempting publication the runner calls `bin/fm-procevent-<adapter>.sh terminal <result-file>` and retires the registration on exit 0 alone, dropping only the exact registration generation captured by its claim and releasing that claim only after removal succeeds under one source boundary; an adapter may stop because the external source ended or because safe continuation requires explicit handler-mediated re-arming, while a missing command, an error, or any other exit keeps the source armed.
...
A terminal registration therefore captures at most one result, is never restarted automatically, and leaves no recurring poll work, while explicit `retire` stays the supported and idempotent path afterwards.
For Lavish that verdict covers every completed feedback poll, an ended session, a missing session, and an ambiguous claimed reply; ordinary open-session feedback is explicitly re-armed with its exact sequence plus `--agent-reply` only after handler revision and durable reconciliation, so a plain poll cannot race ahead of the response that re-enables browser feedback.
The sequence-keyed arm stores its receipt, publishes the registration, and acknowledges the prior result under one source lock; a crash leaves either the result re-announceable or its registered command waiting locally for an idempotent repeat arm.
A continuation receipt moves from ready to claimed before the adapter invokes Lavish, and a replacement runner that sees claimed state emits terminal ambiguity without reposting.
The operator inspects the local session and explicitly records delivered or not-delivered recovery before acknowledging that ambiguity, so a pre-recovery crash preserves its re-announcement and successful recovery prints the exact sequence to acknowledge.
...
An adapter may request a different numeric default only for its normalized output, while any explicit `FM_PROCEVENT_MAX_OUTPUT_BYTES` remains authoritative.
The Lavish adapter requests 16777216 bytes after replacing only a DOM snapshot larger than 16384 encoded bytes with a byte-count marker, which keeps the installed server's complete prompt and decision payload while remaining bounded.
