...
A signal grace window retains only the latest observed signature for each changed status or turn-end source, writes one source-specific durable row per source, and emits a fixed-size batch reason when several sources changed together.
After one notification for a recovery generation is delivered, later normal-mode actionable events join that durable generation without closing another watcher cycle while its handling acknowledgement is outstanding.
If acknowledgement consumes its handled sequence but leaves higher-sequence rows, it republishes downtime and the live watcher or next arm emits one bounded recovery notification for those survivors.
Away mode remains one-shot because its daemon drains every close itself before batching captain-facing delivery.
[`watcher-continuity.md`](watcher-continuity.md#recovery-episode-acknowledgement) owns the exact generation and acknowledgement contract.
...
A stale pane whose matching captain-relevant status was already surfaced is also absorbed when the authoritative current-state read reports `parked`, `blocked`, `done`, or `failed` from a run-step or status-log source.
That exception never absorbs the first decision or blocker event, an unknown or unreadable current state, an endpoint death that makes current state unknown, or a genuinely working worker, so ordinary wedge and deep-inspection detection remain active.
...
Durable batching sits after backend detection and before primary-harness notification delivery, so tmux, Herdr, Zellij, Orca, and cmux share it; Herdr's native transition path reaches the same wake owner rather than a parallel queue.
