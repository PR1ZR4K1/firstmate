...
| `fm-lavish-review.sh`    | Recommend a review surface, confine its private artifact tree, and run every Lavish lifecycle command in one home-scoped runtime envelope |
| `fm-procevent.sh`        | Register, isolate, durably capture, announce, reconcile, acknowledge, and retire blocking process-event sources |
| `fm-procevent-lavish.sh` | Register one prompt-preserving Lavish poll with sequence-keyed reply receipts and explicit ambiguous-delivery recovery |
